...
def test_bulk_dashboard_cursor_and_assignment_index():
    ...
    assignment = client.get("/api/assignment/os-1/assignment-42")
    ...
    with engine.connect() as connection:
        ...
        plan = "\n".join(
            ...
            for row in connection.execute(
                text(
                    "EXPLAIN SELECT * FROM snapshot_event "
                    "WHERE assignment_id=42 AND student_key='202012345' "
                    "ORDER BY occurred_at DESC, id DESC LIMIT 10"
                )
            )
        )
    assert "Index Scan" in plan
    assert "ix_snapshot_assignment_time" in plan
